...
module.exports = async function handler(req, res) {
  try {
    ...
    for (const doc of funcs) {
      ...
      const hMap   = doc.fields?.horarios?.mapValue?.fields;
      if (!hMap) continue;

      const h = hMap[diaHoje]?.mapValue?.fields;
      if (!h || h.folga?.booleanValue) continue;
    ...
    console.error('[Notificar] Erro:', e.message);
    res.status(500).json({ error: e.message });
  }
};

// ── Gera Access Token via JWT ──────────────────────────────────────────────
async function getAccessToken() {
  ...
  // Limpa a chave privada e formata corretamente
...
async function listar(col, token) {
  const r = await fetch(`${FS_URL}/${col}?pageSize=200`, {
...
async function fcm(deviceToken, titulo, corpo, url, accessToken) {
    ...
    body: JSON.stringify({ message: {
      ...
      webpush: {
        notification: {
          ...
          requireInteraction: true, tag: 'ponto'
        },
  ...
  const d = await r.json();
  if (!r.ok) console.warn('[FCM] Erro:', d.error?.message);
  else       console.log('[FCM] Enviado:', titulo);
}
